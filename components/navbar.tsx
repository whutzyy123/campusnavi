...
export function Navbar() {
  ...
  const { currentUser, isAuthenticated, logout: logoutUser } = useAuthStore();
  ...
  return (
    <nav className="sticky top-0 z-navbar w-full border-b border-gray-200 bg-white/95 backdrop-blur-md shadow-sm pointer-events-auto">
      <div className="mx-auto flex h-14 md:h-16 max-w-7xl items-center justify-between gap-2 md:gap-4 px-2 md:px-4">
        {/* 左侧：品牌 Logo + 学校选择器（靠左） */}
        ...
        {/* 中间：POI 搜索条（flex-1 占满剩余空间，两侧分布时自然居中） */}
        <div className="flex flex-1 justify-center min-w-0 mx-2 md:mx-4">
          {currentUser && pois.length > 0 && onSelectPOI ? (
            <POISearchBar pois={pois} onSelectPOI={onSelectPOI} className="w-full min-w-[72px] max-w-xl" />
          ) : null}
        </div>

        {/* 右侧：未登录时显示学校入驻 + 登录；已登录时显示用户菜单（靠右） */}
