...
export default function SchoolsManagementPage() {
  ...
  const fetchSchools = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        notify.error(result.error || "获取学校列表失败");
      }
    } catch (error) {
      console.error("获取学校列表失败:", error);
      notify.error("获取学校列表失败");
    } finally {
  ...
  const handleToggleStatus = async (schoolId: string, isActive: boolean) => {
    ...
    const toastId = notify.loading(`正在${actionText}学校...`);

    const status = isActive ? "ACTIVE" : "INACTIVE";
    const result = await updateSchoolStatus(schoolId, status);

    if (result.success) {
      notify.success(`学校已${actionText}`, { id: toastId });
      setActionMenuOpen(null);
      setDropdownPosition(null);
      await fetchSchools();
    } else {
      notify.error(result.error || "操作失败", { id: toastId });
    }
  };
  ...
  const handleSaveEdit = async () => {
    if (!editingSchool || !editName.trim()) {
      notify.error("学校名称不能为空");
      return;
    }

    const toastId = notify.loading("正在更新学校信息...");
    ...
    try {
      ...
      notify.success("学校信息更新成功", { id: toastId });
      ...
      notify.error((error as Error).message || "更新失败", { id: toastId });
    }
  };
  ...
  const handleCreateSchool = async () => {
    ...
    try {
      ...
      notify.error((error as Error).message || "创建失败");
    } finally {
  ...
  const handleGenerateInviteAfterCreate = async () => {
    ...
    try {
      ...
      notify.error((error as Error).message || "生成失败");
    } finally {
  ...
  const handleDeleteSchool = (school: School) => {
    openConfirm({
      title: "删除学校",
      description: `确定要彻底删除学校「${school.name}」(代码: ${school.schoolCode}) 吗？此操作将永久删除该校及其所有关联数据（用户、POI、邀请码等），且不可恢复。`,
      variant: "danger",
      confirmText: "删除",
      onConfirm: async () => {
        const toastId = notify.loading("正在删除学校...");
        setActionMenuOpen(null);
        const result = await deleteSchool(school.id);
        if (result.success) {
          notify.success("学校已永久删除", { id: toastId });
          await fetchSchools();
        } else {
    ...
    <AuthGuard requiredRole="SUPER_ADMIN">
      <AdminLayout>
        <div className="p-6">
          ...
          {/* 编辑学校弹窗 */}
          <Modal
            isOpen={showEditModal && !!editingSchool}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-2">
                      ...
                      <Button
                        ...
                        disabled={!newSchoolName.trim() || !newSchoolCode.trim()}
                        onClick={handleCreateSchool}
                        className="flex-1"
                      >
                        {!isCreatingSchool ? <Save className="h-4 w-4" /> : null}
