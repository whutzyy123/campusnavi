...
const LIVE_STATUS_POINTS_DAILY_LIMIT = 10;
const LIVE_STATUS_POINTS_WINDOW_MS = 24 * 60 * 60 * 1000;
const LIVE_STATUS_REPORT_COOLDOWN_MS = 10 * 60 * 1000;
...
export async function reportLiveStatus(
  ...
  try {
    ...
    // 全局冷却：同一用户任意两次上报之间至少间隔 10 分钟（跨 POI 共享）
    const latestUserReport = await prisma.liveStatus.findFirst({
    ...
    if (latestUserReport) {
      ...
      const lastMs = latestUserReport.createdAt.getTime();
      const elapsedMs = nowMs - lastMs;
      if (elapsedMs < LIVE_STATUS_REPORT_COOLDOWN_MS) {
        const remainingSeconds = Math.ceil(
          (LIVE_STATUS_REPORT_COOLDOWN_MS - elapsedMs) / 1000
        );
        return {
          success: false,
          error: `上报过于频繁，请在 ${remainingSeconds} 秒后重试`,
        };
      }
    }
    ...
    const liveStatus = await prisma.$transaction(async (tx) => {
      const now = new Date();
      const windowStart = new Date(now.getTime() - LIVE_STATUS_POINTS_WINDOW_MS);

      // 按“通过上报获取积分”统计最近 24 小时已获得次数，上限 10
      const recentReportCount = await tx.liveStatus.count({
        where: {
          userId: auth.userId,
      ...
      const created = await tx.liveStatus.create({
        data: {
          poiId: poi.id,
      ...
      if (recentReportCount < LIVE_STATUS_POINTS_DAILY_LIMIT) {
