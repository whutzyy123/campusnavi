...
import { calculateStatusStatistics } from "@/lib/geo/poi-utils";

function normalizeCoordinate(value: number | { toNumber: () => number }): number {
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        isHidden: updated.reportCount >= 5,
      },
    };
  } catch (err) {
...
export async function getPOIsBySchool(
  ...
  try {
    ...
    // 批量查询所有 POI 的实时状态（避免 N+1 查询）
    const poiIds = pois.map((p) => p.id);
    const allStatuses = await prisma.liveStatus.findMany({
      where: {
        poiId: { in: poiIds },
        schoolId,
        expiresAt: { gt: now },
      ...
      select: { val: true, statusType: true, expiresAt: true, createdAt: true, poiId: true },
      orderBy: { createdAt: "desc" },
    });

    // 按 POI ID 分组，每组保留最新一条
    const latestStatusByPoi = new Map<string, { val: number; statusType: string; expiresAt: Date; createdAt: Date }>();
    for (const status of allStatuses) {
      if (!latestStatusByPoi.has(status.poiId)) {
        latestStatusByPoi.set(status.poiId, status);
      }
    }

    // 合并 POI 数据与状态
    const poisWithStatus = pois.map((poi) => {
      const recentStatuses = latestStatusByPoi.get(poi.id)
        ? [latestStatusByPoi.get(poi.id)!]
        : [];
      ...
      return {
        ...
        lat: normalizeCoordinate(poi.lat),
        lng: normalizeCoordinate(poi.lng),
...
export async function getPOIDetail(id: string): Promise<POIActionResult<{ poi: POIDetail }>> {
  try {
    ...
    type POIRow = { id: string; schoolId: string; parentId: string | null; name: string; categoryRef: { id: string; name: string } | null; category: string | null; categoryId: string | null; lat: number | { toNumber: () => number }; lng: number | { toNumber: () => number }; isOfficial: boolean; description: string | null; imageUrl: string | null; reportCount: number; createdAt: Date; updatedAt: Date };
  ...
  try {
    ...
    if (input.imageUrl !== undefined) {
      if (poi.imageUrl && poi.imageUrl !== input.imageUrl) {
        const { deleteImageFromStorage } = await import("@/lib/actions/upload");
    ...
    return {
      ...
      data: {
        poi: {
          ...
          lat: normalizeCoordinate(updatedPOI.lat),
          lng: normalizeCoordinate(updatedPOI.lng),
