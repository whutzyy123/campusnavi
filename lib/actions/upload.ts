 ...
 * 任何已登录用户可上传，校验：image/jpeg、image/png、image/webp，最大 2MB
 */
export async function uploadSquarePostImage(formData: FormData): Promise<UploadResult> {
  try {
    const auth = await getAuthCookie();
    if (!auth?.userId) {
      return { success: false, error: "请先登录" };
    ...
    const ext = EXT_MAP[file.type] || "jpg";
    const pathname = `square-posts/${Date.now()}-${Math.random().toString(36).slice(2, 10)}.${ext}`;
    ...
    console.error("广场帖子图片上传失败:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "上传失败，请重试",
    };
