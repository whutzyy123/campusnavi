    ...
    try {
      ...
      if (!(window as any).AMapLoader) {
        ...
        await new Promise<void>((loaderResolve, loaderReject) => {
          ...
          loaderScript.onerror = () => loaderReject(new Error("高德地图加载器脚本加载失败"));
      ...
      reject(
        new Error(
          `高德地图 SDK 加载失败：${error instanceof Error ? error.message : "未知错误"}`
        )
      );
    }
  });
...
export function getAMapInstance(): any {
  if (typeof window === "undefined" || !window.AMap) {
    throw new Error("地图 SDK 尚未加载，请先调用 loadAMap()");
  }
  return window.AMap;
}
...
export function loadAMapPlugin(pluginName: string, timeout: number = 10000): Promise<void> {
  if (typeof window === "undefined") {
    return Promise.reject(new Error("非浏览器环境，无法加载地图"));
  }

  // 确保主脚本已加载
  if (!window.AMap) {
    return Promise.reject(new Error("地图 SDK 尚未加载，请先调用 loadAMap()"));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      try {
        AMap.plugin(pluginName, () => {
          ...
          const retryCheck = () => {
            ...
            if (retryCount < maxRetries) {
              ...
              reject(
                new Error(
                  `插件 ${pluginName} 加载失败：重试 ${maxRetries} 次后仍未就绪`
                )
              );
            }
          };
        ...
        reject(
          new Error(
            `插件 ${pluginName} 加载失败：${error instanceof Error ? error.message : "未知错误"}`
          )
        );
      }
...
export class CoordinateConverter {
  ...
  static formatCoordinate(lng: number, lat: number): [number, number] {
    if (!this.isValidGCJ02(lng, lat)) {
      throw new Error(`GCJ-02 坐标无效：[${lng}, ${lat}]`);
    }
    return [lng, lat];
  }
  ...
  static parseCoordinate(coord: [number, number]): { lng: number; lat: number } {
    if (!Array.isArray(coord) || coord.length !== 2) {
      throw new Error(`坐标数组无效：${JSON.stringify(coord)}`);
    }
    const [lng, lat] = coord;
    if (!this.isValidGCJ02(lng, lat)) {
      throw new Error(`坐标数组内 GCJ-02 无效：[${lng}, ${lat}]`);
    }
    return { lng, lat };
  }
}
