...
let sensitiveWordsCache: SensitiveWordsCache | null = null;
...
async function getSensitiveWords(): Promise<string[]> {
  ...
  if (
    sensitiveWordsCache &&
    now - sensitiveWordsCache.timestamp < CACHE_TTL_MS
  ) {
    return sensitiveWordsCache.words;
  }

  const records = await prisma.sensitiveWord.findMany({
    select: { keyword: true },
  });

  sensitiveWordsCache = {
    words: records.map((r) => r.keyword.toLowerCase()),
    timestamp: now,
  };
